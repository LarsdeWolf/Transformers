...
class MoE(nn.Module):
    ...
    computes scores of shape (tokens, num_experts). Top-k scores are selected and rescaled,
    such that for each output token consists of the weighted combination of the top-k experts output.
...
class PretrainedVAE(nn.Module):
    '''
    Load VAE model from huggingface
    '''
    def __init__(self, path: str):
        super().__init__()
        self.vae = AutoencoderTiny.from_pretrained(path)
